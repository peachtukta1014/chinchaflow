...
| 🌐 Web search | สั่ง "ค้นหา..." → จีจี้ดึงข้อมูลจากเว็บจริงผ่าน DeepSeek + web plugin |
...
**โมเดล (4-tier ผ่าน OpenRouter):**

| ชั้น | โมเดล | ใช้ตอน |
|------|-------|--------|
| 🟢 Flash | `deepseek/deepseek-v4-flash` | classify intent + แชททั่วไป (เร็ว/ถูก) |
| 🟡 Pro | `deepseek/deepseek-v4-pro` | agentic loop เขียนโค้ดจริง (commit/PR) |
| 🔵 Vision | `openai/gpt-4o-mini` | มีรูปแนบ (screenshot, สลิป) |
| 🌐 Search | `deepseek/deepseek-chat` + web plugin | ค้นเว็บ on-demand |
...
| `deploy-functions.yml` | `apps/webhook-core` | deploy Cloud Functions + sync agent docs เข้า Firestore |
| `deploy-rules.yml` | `firestore*.rules`, `storage.rules` | deploy security rules |
| `pr-verify.yml` | ทุก PR | smoke test + build + comment ผลกลับ PR (merge โดยพีชหรือสั่งพี่ซี) |
| `ai-workflow-trigger.yml` | `repository_dispatch (ai-code-action)` จาก Flash | รัน Pro Agent agentic loop — อ่านโค้ด → แก้ → commit → เปิด PR |
| `auto-changelog.yml` | PR merge เข้า `main` | เขียน `docs/AGENT_CHANGELOG_TH.md` + `apps/<app>/CHANGELOG.md` อัตโนมัติ |
| `sync-ai-constants.yml` | แก้ `agentTools.js` | sync ค่า MAX_ITERATIONS/CHECKPOINT เข้าเอกสารที่เกี่ยวข้องอัตโนมัติ |
...
- [docs/AI_AGENT_SYSTEM.md](docs/AI_AGENT_SYSTEM.md) — สถาปัตยกรรม AI ครบชุด (Flash/Pro/keys/flow diagram)
...
- [FLASH.md](FLASH.md) · [PRO.md](PRO.md) — ตัวตน Flash/Pro Agent
- [AGENTS.md](AGENTS.md) · [CLAUDE.md](CLAUDE.md) — กฎ monorepo + กฎ Claude Code
