...
| `patch_file` | `path`, `find`, `replace_with`, `reason` | แก้ข้อความในไฟล์ (in-memory) — **`find` ต้อง unique ในไฟล์** ถ้าเจอซ้ำหลายที่จะถูกปฏิเสธ (กัน patch ผิดจุดแบบเงียบๆ) ให้เพิ่ม context รอบข้าง |
...
| `trigger_deploy` | `app`, `ref?` | trigger GitHub Actions workflow — `app` คือ `chincha-tea\|seafood-pos\|webhook-core\|ai-chat` (workflow file เลือกให้อัตโนมัติ) |
| `get_skill` | `name` | โหลด skill doc — `scope-*` (กฎ/gotchas ของ scope) หรือ `run-*` (คำสั่ง verify ของแอป) |
| `report_no_action_needed` | `summary`, `need_more_info?` | รายงานว่าไม่ต้องแก้ แล้วจบ |
...
CHECKPOINT_INTERVAL = 7    — สรุปความคืบหน้าซ้ำทุก 7 รอบ (ไม่ใช่ครั้งเดียว) แล้วดำเนินต่อทันที
```
...
| Emergency commit | ถึง MAX=22 มีไฟล์ staged | commit `[WIP]` แล้วหยุด — PR URL ที่ได้จะแนบไปในข้อความ error ที่ส่งกลับพีชด้วยเสมอ (ไม่ทิ้งเงียบๆ) |
...
- `get_skill("scope-<scope>")` — โหลดกฎ/gotchas ของ scope นี้ก่อน (เช่น `scope-seafood`, `scope-tea`)
- `read_file` ไฟล์ใน `files_hint[0].path` ก่อนเสมอ (fn บอก function ที่ต้องแก้)
- อ่าน `AGENTS.md` + scope AGENTS.md จาก `fetchAgentDocs()`
- `get_skill("run-<scope>")` แยกต่างหาก — โหลดตอนขั้น 3 (verify) เท่านั้น ไม่ใช่ตัวเดียวกับ `scope-<scope>`

### 2. แก้โค้ด
- `patch_file` หรือ `write_file` เฉพาะส่วนที่เกี่ยว
- ตรวจ business rules จาก Task Brief
- **ห้ามเขียน/แก้ changelog เอง** — `auto-changelog.yml` เขียนให้อัตโนมัติตอน PR merge (ทั้ง `docs/AGENT_CHANGELOG_TH.md` และ `apps/<app>/CHANGELOG.md`) หน้าที่ Pro คือแค่อ่านก่อนแก้ (ขั้น 1)
