...
## 2) Slack — ผูก repo กับ channel

ใน channel ที่ใช้บ่อย (เช่น `#chincha-flow`) พิมพ์ครั้งเดียว:
...
หลังนี้แค่ `@cursor` + งาน — ไม่ต้องเลือก repo ทุกครั้ง
...
## 3) Repo — environment สำหรับ Cloud Agent (ทำแล้วใน git)
