...
│   │
│   ├── flash/                     # จีจี้ (Flash) — สมองฝั่ง chat
│   │   ├── flashTriggers.js       # quick triggers + classifyAndTranslate + buildTaskBrief
│   │   ├── flashAnalysisLoop.js   # Code Analysis Loop (อ่านโค้ดจริงก่อนสรุป Task Brief)
│   │   ├── flashModels.js         # OpenRouter callers + model constants
│   │   ├── flashPrompts.js · flashContext.js · flashDispatch.js
│   │   └── webSearchTag.js        # จับ/ลบแท็ก [WEB_SEARCH:] (เทสได้)
│   │
...
│
├── scripts/
│   ├── test-*.js                  # เทส logic ~20 ไฟล์ (ไม่ต้อง key) — รันใน pr-verify.yml
│   ├── eval-flash-intents.mjs     # สนามสอบ AI classifier (ยิง LLM จริง) — รันใน ai-eval.yml
│   └── sync-agent-docs.cjs · ack-pro-agent.cjs · ...  # CI helpers
│
└── package.json
```
...
| `pr-verify.yml` | ทุก PR | smoke test + test scripts webhook-core + syntax check |
| `ai-eval.yml` | PR แตะ `flash/`/`aiChatAgent.js` หรือรันมือ | สอบ AI classifier ด้วยข้อสอบจริง → ตารางผลใน PR |
