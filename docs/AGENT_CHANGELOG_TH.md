...
### 2026-06-11 — ชา: ชี้ชัดสูตรสต๊อกแก้วสำหรับพนักงาน

- **ปัญหา/คำขอ:** Peach ย้ำว่าพนักงานต้องบันทึกจำนวนแก้วที่นำขึ้นมาเตรียมขาย/เติมเข้า และให้ระบบคำนวณแก้วคงเหลือจากยอดขายต่อวันให้ชัดเจน
- **แก้แล้ว:** เพิ่มช่องแสดง `แก้วเตรียมขายรวม` และบันทึก `preparedCups`; ปรับ label เป็น `นำแก้วขึ้นมาเตรียมขาย / เติมเข้า`, `จำนวนแก้วขายออกวันนี้`, `แก้วคงเหลือวันนี้` พร้อม hint สูตร `ยกมา + เติมเข้า − ขายออก = คงเหลือ`
- **ไฟล์/จุดสำคัญ:** `apps/chincha-tea/src/screens/ExpensesTab.jsx`, `apps/chincha-tea/src/lib/i18n.js`, `docs/ARCHITECTURE_TH.md`, `docs/PROJECT_STRUCTURE.md`
- **พฤติกรรมหลังแก้:** พนักงานที่ approved บันทึกแก้วได้ในแท็บค่าใช้จ่าย; ระบบคำนวณคงเหลือและยกยอดไปวันถัดไปผ่าน `dailyCupStocks`
- **ถ้าพังอีก ให้เช็กก่อน:** `dailyCupStocks/{YYYY-MM-DD}` ต้องมี `openingCups`, `addedCups`, `preparedCups`, `cupsSold`, `remainingCups`

### 2026-06-11 — ชา: รีเวิร์กแท็บค่าใช้จ่ายเป็นสรุปหน้าร้าน + สต๊อกแก้ว

- **ปัญหา/คำขอ:** PR ก่อนยังเป็นช่องวางแชท/รายการจ่ายทั่วไป ไม่ตรงฟอร์มที่ Peach ต้องการ: เงินสด, โอน, จ่ายออกหน้าร้าน, แก้วขาย, รวม, ซื้อของจากสั่งซื้อ และสต๊อกแก้วเปล่าสำหรับพนักงาน
- **แก้แล้ว:** แท็บค่าใช้จ่ายกลายเป็น daily close แบบมีช่องชัดเจน + auto-fill จากบิลขาย/รายการสั่งซื้อ, บันทึก `dailyShopSummaries`, sync จ่ายออกหน้าร้านเข้า `dailyExpenses`, และเพิ่ม `dailyCupStocks` สำหรับยกยอดแก้วคงเหลือไปวันถัดไป
- **ไฟล์/จุดสำคัญ:** `apps/chincha-tea/src/screens/ExpensesTab.jsx`, `apps/chincha-tea/src/lib/i18n.js`, `firestore.rules`, `docs/ARCHITECTURE_TH.md`, `docs/PROJECT_STRUCTURE.md`
- **พฤติกรรมหลังแก้:** พนักงาน/แอดมินเลือกวันย้อนหลังได้, กรอก/วางสรุปจากแชทเพื่อเติมช่อง, บันทึกสรุปหน้าร้านและสต๊อกแก้วแยกกัน; คงเหลือแก้ววันก่อนถูกดึงมาเป็นยอดยกมาเมื่อเปิดวันถัดไป
- **ถ้าพังอีก ให้เช็กก่อน:** rules ของ `dailyShopSummaries`/`dailyCupStocks`, doc id `storefront-summary-YYYY-MM-DD`/`dailyCupStocks/YYYY-MM-DD`, และว่ายอดซื้อเข้าร้านถูก mark purchased ในแท็บสั่งของแล้ว
