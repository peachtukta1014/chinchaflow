...
### 2026-06-03 — ชา: ติ๊กมาทำงานอัตโนมัติเมื่อบันทึกขาย (แท็บตัดวัน)

- **ปัญหา/คำขอ:** แท็บตัดวัน/รอบ 15 วัน — อยากให้ติ๊กลูกน้องมาทำงานอัตโนมัติเมื่อวันนั้นมีการบันทึกขายครั้งแรก
- **แก้แล้ว:** หลัง `saveTeaOrder` เรียก `ensurePrimaryStaffPresentOnSale` · Firestore rules อนุญาต `markedSource: sale` สำหรับผู้ใช้ที่อนุมัติแล้ว
- **ไฟล์/จุดสำคัญ:** `orderService.js`, `staffAttendanceService.js`, `firestore.rules`, `i18n.js`
- **พฤติกรรมหลังแก้:** บันทึกออเดอร์วันใด → ถ้ายังไม่ติ๊กพนักงานหลัก (`2004@chincha.pos`) จะติ๊กให้ · แอดมินยกเลิกติ๊กมือได้ · ขายรอบถัดไปไม่ทับถ้าติ๊กแล้ว
- **ถ้าพังอีก ให้เช็กก่อน:** deploy **rules + hosting** · `dailyStaffAttendance` · พนักงานหลักต้อง approved + role staff
