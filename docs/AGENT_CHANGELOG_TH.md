...
พีชสังเกตว่า agent changelog/docs ไม่ sync อัตโนมัติหลัง deploy — เช็กแล้วเจอสาเหตุจริง:

PR #451 เปลี่ยนชื่อ constant ใน `agentTools.js` จาก `SUMMARY_CHECKPOINT` (ครั้งเดียว) → `CHECKPOINT_INTERVAL` (ซ้ำทุก 7 รอบ) แต่ `.github/workflows/sync-ai-constants.yml` ยัง grep หาชื่อเก่าอยู่ พอ merge แล้ว CI รัน auto-sync ก็หาค่าไม่เจอ (regex ไม่ match) → เขียนค่าว่างทับ `docs/AI_AGENT_SYSTEM.md` (`SUMMARY_CHECKPOINT =     — รอบ 9...` และ `CHECKPOINT=` ว่างในไดอะแกรม/key files map)

## การเปลี่ยนแปลง

- `.github/workflows/sync-ai-constants.yml` — เปลี่ยน grep pattern จาก `SUMMARY_CHECKPOINT` → `CHECKPOINT_INTERVAL` (ทั้ง extract step และ sed patch step)
- `docs/AI_AGENT_SYSTEM.md` — แก้ label + เติมค่าที่ว่างอยู่ 3 จุดให้ตรงกับโค้ดจริง (`CHECKPOINT_INTERVAL = 7`) — จุดนี้ CI จะไม่ auto-fix ให้เพราะ path filter ของ workflow คือ `agentTools.js` เท่านั้น (ไฟล์นั้นไม่ได้แก้ใน PR นี้)

## Test plan

- [x] `python3 -c "import yaml; yaml.safe_load(...)"` ยืนยัน workflow YAML ยัง valid
- [ ] ครั้งหน้าที่แก้ `agentTools.js` (MAX_ITERATIONS/CHECKPOINT_INTERVAL) → เช็ก GitHub Actions log ของ `sync-ai-constants` ว่า extract ค่าไม่ว่าง

Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>

https://claude.ai/code/session_01KxKmqRN4gJkKWP6aRb1mxd

---
_Generated by [Claude Code](https://claude.

## 2026-07-01 — feat: Flash Code Analysis Loop — อ่านโค้ดจริงก่อนสรุป Task Brief (PR #452)

## สรุป

เดิม Flash (`classifyAndTranslate`) เดา `target_behavior`/`logic_constraints`/`files_hint` ทั้งหมดจากบทสนทนาอย่างเดียว — จุดที่มีอยู่แล้ว (`fetchRepoFiles` ใน `aiChatAgent.js`) ถูกเรียกใช้แค่ **เช็กว่า path มีอยู่จริงไหม** (M1, PR #450) เนื้อไฟล์ที่ดึงมาได้ถูกทิ้งไม่ได้ใช้วิเคราะห์อะไรต่อ ทั้งที่ `docs/AI_AGENT_SYSTEM.md` เดิมเขียนไว้ว่า "แนบเข้า Task Brief" — เอกสารกับโค้ดจริงไม่ตรงกัน

PR นี้ให้ Flash **อ่านโค้ดจริงก่อนสรุปงาน** ไม่ใช่แค่เดา ตามที่คุยกันว่า Flash ต้องทำหน้าที่หัวหน้าคุมงานที่ตรวจโค้ดจริงก่อนสั่ง Pro ไปแก้
...
- เพิ่ม `apps/webhook-core/src/flash/flashAnalysisLoop.js` — read-only agentic loop ใหม่สำหรับ Flash (`runFlashAnalysisLoop`)
  ...
  - `MAX_ITERATIONS = 6` (สำรวจเบื้องต้น ไม่ใช่แก้โค้ด), timeout ต่อรอบ 60s
  - ผูก **`GH_PAT_READ` เท่านั้น** — read-only ล้วน ไม่มี patch/write/commit เหมือน Pro

---

## 2026-07-01 — fix: sync-ai-constants.yml ยังอ้าง SUMMARY_CHECKPOINT ที่ถูกลบไปแล้ว

### อาการ
PR #451 เปลี่ยนชื่อ constant ใน `agentTools.js` จาก `SUMMARY_CHECKPOINT` (ครั้งเดียว) → `CHECKPOINT_INTERVAL` (ซ้ำทุก 7 รอบ) แต่ลืมอัปเดต `.github/workflows/sync-ai-constants.yml` ที่ grep หา `SUMMARY_CHECKPOINT` — พอ CI รันหลัง merge ก็หา match ไม่เจอ ได้ค่าว่าง แล้วเขียนทับ `docs/AI_AGENT_SYSTEM.md` ให้ค่า checkpoint เป็นช่องว่าง (`SUMMARY_CHECKPOINT =     — รอบ 9...` และ `CHECKPOINT=` ว่างในไดอะแกรม/key files map)

### วิธีแก้
- `.github/workflows/sync-ai-constants.yml` — เปลี่ยน grep pattern จาก `SUMMARY_CHECKPOINT = )\d+` → `CHECKPOINT_INTERVAL = )\d+` (extract step + sed patch step)
- `docs/AI_AGENT_SYSTEM.md` — แก้ label ในโค้ดบล็อก section 4 จาก `SUMMARY_CHECKPOINT = ` (blank) → `CHECKPOINT_INTERVAL = 7   — สรุปความคืบหน้าซ้ำทุก 7 รอบ แล้วดำเนินต่อทันที (ไม่ใช่ครั้งเดียว)` ให้ตรงกับโค้ดจริง + เติมค่า `CHECKPOINT=7` ที่ว่างอยู่ 2 จุด (diagram + key files map) — จุดนี้ CI จะไม่ auto-fix ให้เพราะ path filter ของ workflow คือ `agentTools.js` เท่านั้น (ไฟล์นี้ไม่ได้แก้ในรอบนี้)

### วิธีตรวจถ้าพัง
- แก้ `MAX_ITERATIONS`/`CHECKPOINT_INTERVAL` ใน `agentTools.js` รอบหน้า → ดู GitHub Actions log ของ `sync-ai-constants` ว่า extract ได้ค่าไม่ว่าง (`📌 MAX_ITERATIONS=X CHECKPOINT_INTERVAL=Y`)
...
|-----|------|-------|--------|
| ข้อ 2 | `aiChatAgent.js:56` | clearResult ก่อน res.json → ผลงาน Pro หายถ้า mobile หลุด | สลับลำดับ: respond ก่อน แล้ว cleanup |
| ข้อ 5A | `aiChatAgent.js:111` | clearPendingAction ก่อน dispatch → brief หายถ้า GitHub API ล้ม | ย้าย clearPendingAction หลัง dispatch สำเร็จ |
| ข้อ 4 | `aiWorkflowAgent.js:374` | Pro เห็นแค่ 25 ไฟล์แรก ทั้งที่ seafood มี 88 | เพิ่มเป็น 50 ไฟล์ |
| ข้อ 3 | `progressTracker.js:136` | schema comment ขาด key `search?` | เพิ่ม `search?` ใน comment |

## 2026-06-30 — feat: run skills ทั้ง 4 apps — seafood-pos, chincha-tea, ai-chat, webhook-core (PR #445)

## สิ่งที่เพิ่ม

`/run-<app>` skill สำหรับทุก app ใน monorepo — ทดสอบจริงในสภาพ headless container session นี้

| Skill | ไฟล์ | วิธี drive |
|-------|------|-----------|
| `/run-seafood-pos` | `apps/seafood-pos/.claude/skills/run-seafood-pos/SKILL.md` | vite dev + chromium screenshot + smoke-test.mjs |
| `/run-chincha-tea` | `apps/chincha-tea/.claude/skills/run-chincha-tea/SKILL.md` | vite dev + chromium screenshot |
| `/run-ai-chat` | `apps/ai-chat/.claude/skills/run-ai-chat/SKILL.md` | vite dev + chromium screenshot |
| `/run-webhook-core` | `apps/webhook-core/.claude/skills/run-webhook-core/SKILL.md` | syntax check + smoke test (ไม่มี dev server) |

## สิ่งที่ทดสอบในสภาพนี้ (session นี้ container นี้)

- **seafood-pos**: dev server ขึ้น port 5174 ✅, screenshot เห็น "กำลังโหลด..." (Firebase ไม่มี key = expected) ✅, smoke-test.mjs 14/15 ✅
- **chincha-tea**: dev server ขึ้น port 5175 ✅, screenshot เห็น login page + CHINCHA logo + language selector (ไทย/မြန်မာ/EN) ✅
- **ai-chat**: dev server ขึ้น port 5176 ✅, screenshot เห็น จจี้ avatar + "เข้าสู่ระบบด้วย Google" ✅
- **webhook-core**: syntax check ผ่าน 54 ไฟล์ ✅ (Cloud Functions ไม่มี standalone dev server)

## Gotchas ที่พบ (เขียนไว้ใน SKILL.md)

- ต้องใช้ `../../node_modules/.bin/vite` (root vite ไม่มี local)
- ต้องเพิ่ม `--host 0.0.0.0` เสมอเมื่อ agent ต้องการ chromium เข้าถึง
- chincha-tea `npm run dev` ไม่มี `--host` → binds localhost เท่านั้น
- webhook-core ไม่มี dev server — test ผ่าน syntax check + script ตร�

## 2026-06-30 — fix: concurrency group ป้องกัน race condition บน main (auto-commit workflows) (PR #444)

## สาเหตุ (root cause จริง)

`git pull --rebase` ใน PR #443 ไม่ได้แก้ race ได้จริง — เพราะทั้งสอง workflow (sync-project-tree + auto-changelog) อาจ pull พร้อมกันในช่วงเวลาเดียว เห็น main SHA เดิมทั้งคู่ แล้วก็ push พร้อมกัน ตัวที่ช้ากว่าถูก reject:

```
! [remote rejected] main -> main (cannot lock ref: is at b300... but expected 5e47...)
```

## วิธีแก้

เพิ่ม `concurrency` group เดียวกัน (`auto-commit-main`) ในทุก workflow ที่ auto-commit ลง main:
...
- `cancel-in-progress: false` → ให้คิวรอแทนที่จะ cancel กัน ไม่มี changelog entry หายแน่นอน
- ทั้ง 3 workflow ใช้ group เดียวกัน → serialize กัน วิ่งทีละตัว

| Workflow | ไฟล์ |
|----------|------|
| Sync Project Tree | `.github/workflows/sync-project-tree.yml` |
| Auto Changelog | `.github/workflows/auto-changelog.yml` |
| Sync AI Constants | `.github/workflows/sync-ai-constants.yml` |

---
_Generated by [Claude Code](https://claude.ai/code/session_012EjL66R6dMhsCUt4RxUALS)_


## 2026-06-30 — refactor: รวม AI docs + auto-sync constants CI + auto-changelog CI (PR #442)

## สรุปการเปลี่ยนแปลง

แก้ปัญหา docs ซ้ำซ้อนและค่า constants stale ระยะยาว

### ลด docs AI จาก 4 ไฟล์ → 1 ไฟล์

| เดิม | ใหม่ |
|------|------|
| `docs/AI_AGENT_DIAGRAM.md` (209 บรรทัด) | รวมเข้า `docs/AI_AGENT_SYSTEM.md` |
| `docs/AI_AGENT_KEY_FILES.md` (216 บรรทัด) | รวมเข้า `docs/AI_AGENT_SYSTEM.md` |

`docs/AI_AGENT_SYSTEM.md` (ไฟล์ใหม่) ออกแบบให้:
- Flash อ่านส่วนแรก 4,000 chars = 3-agent overview + security + 5 scopes + loop limits
- Pro + พี่ซีอ่านได้ทั้งหมด (full flowchart + key files map + API keys)

### Auto-sync constants (ไม่ต้อง manual อีก)

`.github/workflows/sync-ai-constants.yml` — ทุกครั้งที่ `agentTools.js` เปลี่ยน:
- grep `MAX_ITERATIONS` + `SUMMARY_CHECKPOINT` จากโค้ดจริง
- sed patch ค่าเข้า `docs/AI_AGENT_SYSTEM.md` อัตโนมัติ
- commit `[skip ci]` → ค่า docs ตรงโค้ดเสมอ

### Auto-changelog (ไม่ต้องเขียน manual อีก)

`.github/workflows/auto-changelog.yml` — ทุก PR merge เข้า main:
- ดึง PR title + body
- prepend เข้า `docs/AGENT_CHANGELOG_TH.md` อัตโนมัติ
- commit `[skip ci]`

### แก้อื่นๆ

- `PRO.md:129` — fix stale `SUMMARY_CHECKPOINT = 25` → `9`
- `docs/ARCHITECTURE_TH.md` — ตัด AI table ออก เหลือ pointer ไป AI_AGENT_SYSTEM.md
- `docs/AGENT_HANDBOOK_TH.md` — แก้ `.jiiji/` reference → `.skill/`, เพิ่ม AI_AGENT_SYSTEM.md ในตาราง
- `flashContext.js` + `sync-agent-docs.cjs` — ชี้ไป AI_AGENT_SYSTEM.md แทน AI_AGENT_DIAGRAM.md

---
_Generated by [Claude Code](h
