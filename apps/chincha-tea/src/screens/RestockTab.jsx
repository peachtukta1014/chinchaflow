...
export function RestockTab({ member, t, lang = 'th', onRestockListChange }) {
  ...
  const submitRestockList = useCallback(async (listToSubmit) => {
      ...
      const created = await fsPost('restocks', {
        ...
        items: listToSubmit.map((i) => {
          ...
          return {
            ...
            qty: i.qty,
            status: i.status,
            purchaseUnitPrice: latestPriceForName(i.name) || catalogUnitPrice(catalogItem) || 0,
    ...
    setSaving(false);
  }, [catalogByKey, dateKey, latestPriceForName, member, notifyRestockChange, refreshCatalog, refreshRecent, t]);
