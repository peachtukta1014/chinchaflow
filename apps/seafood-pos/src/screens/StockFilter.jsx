...
export default function StockFilter({
  ...
  const bySizeShrimpCost =
    sizeMode === 'by_size'
      ? calcBySizeShrimpCost({ A: sizeAKg, B: sizeBKg, C: sizeCKg, priceA, priceB, priceC })
      : 0;
  ...
  function buildSizeBreakdown() {
    ...
    return buildBySizeBreakdown({ A: sizeAKg, B: sizeBKg, C: sizeCKg, priceA, priceB, priceC });
  }
  ...
  const switchSizeMode = (mode) => {
    ...
    if (mode === 'by_size') {
      ...
      setSizeA(''); setSizeB(''); setSizeC('');
      setPriceA(''); setPriceB(''); setPriceC('');
    }
  };
  ...
  const handleReceiveLive = async () => {
    ...
    if (sizeMode === 'by_size') {
      const missing = missingSizePriceLabel({ A: sizeA, B: sizeB, C: sizeC, priceA, priceB, priceC });
