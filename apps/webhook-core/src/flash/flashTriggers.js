...
ขั้นตอนตัดสิน intent (เช็คตามลำดับ 1→3 เจอข้อไหนก่อนใช้ข้อนั้น):
1. บอกอาการระบบผิดปกติ — พัง/ค้าง/error/โหลดไม่ได้/ไม่ตอบ/ไม่เข้าระบบ/หายไป/แสดงผิด/อัพโหลดไม่ได้ (แม้ขึ้นต้นด้วย "ทำไม" หรือลงท้าย "ช่วยดูหน่อย/ตรวจสอบที") → "code-action"
2. สั่งเปลี่ยนแอป/โค้ด — แก้/เพิ่ม/ลบ/เปลี่ยน/ปรับ ปุ่ม สี ข้อความ ฟีเจอร์ เวลาแจ้งเตือน ฯลฯ → "code-action"
3. ถามข้อมูล/ยอดขาย/ขอสรุปตัวเลข/ขอความเห็น/ขอคำอธิบาย/ทักทาย/ขอบคุณ (ระบบปกติดี) → "chat"

ตัวอย่าง: "ทำไมสรุปยอดไม่ส่งเข้า LINE ช่วยดูที"→code-action(ข้อ1) · "สรุปยอดเมื่อคืนให้หน่อย"→chat(ข้อ3) · "เปลี่ยนสีปุ่มเป็นเขียว"→code-action(ข้อ2) · "ลูกหนี้ค้างเท่าไหร่"→chat(ข้อ3)
