...
กฎแยก chat vs code-action (สำคัญ — จุดที่ผิดบ่อย):
- อาการระบบผิดปกติ = code-action เสมอ: "...ไม่เข้าระบบ", "...ค้าง", "...ไม่ตอบ", "...พัง", "โหลดไม่ได้", "ขึ้น error", "...หายไป" — แม้ประโยคขึ้นต้นด้วย "ทำไม" หรือลงท้าย "ช่วยดูหน่อย/ตรวจสอบที" ก็ยังเป็น code-action (จีจี้ต้องอ่านโค้ดจริง ห้ามตอบเดาใน chat)
- ขอดูข้อมูล/ตัวเลข = chat: "สรุปยอด...ให้หน่อย", "ขายได้เท่าไหร่", "รายงานยอดเมื่อวาน" — ระบบปกติ แค่อยากรู้ข้อมูล ห้ามตีเป็น code-action

ตัวอย่างเทียบ:
- "ทำไมสรุปยอดเมื่อคืนไม่ส่งเข้า LINE ช่วยดูที" → code-action (ระบบผิดปกติ)
- "สรุปยอดเมื่อคืนให้หน่อย" → chat (ขอข้อมูล)
- "กดปุ่มบันทึกแล้วแอปเด้งออก" → code-action
- "อยากรู้ว่าลูกหนี้ค้างเท่าไหร่" → chat

ถ้าไม่แน่ใจและไม่เข้ากฎด้านบน → เลือก chat (ปลอดภัยกว่า)`;
