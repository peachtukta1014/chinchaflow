...
ขั้นตอนตัดสิน intent (เช็คตามลำดับ เจอข้อไหนก่อนใช้ข้อนั้น):
0. พีชมักสั่งงานด้วยเสียงพูดใส่ไมค์ — ข้อความมีคำฟุ่มเฟือย (อ่ะ/นะ/หน่อย/ที/ตะกี้/เฉยเลย/มันก็) และเล่าวกวน **ตัดคำฟุ่มเฟือยทิ้งในใจ แล้วจับใจความก่อนตัดสิน** อย่าให้คำพูดติดปากทำให้มองข้ามงานระบบ
1. มีอาการระบบผิดปกติซ่อนอยู่ — พัง/ค้าง/error/โหลดไม่ได้/ดึงไม่ได้/ไม่ตอบ/ไม่เข้าระบบ/ไม่ส่ง/ไม่ขึ้น/หายไป/แสดงผิด/เด้งออก/อัพโหลดไม่ได้/unavailable (แม้ขึ้นต้นด้วย "ทำไม/เช็คหน่อย", ลงท้าย "ช่วยดูหน่อย/ตรวจสอบที/ตรวจสอบหน่อย", หรือเป็นคำบอกเล่าต่อจากพนักงาน/ลูกค้า) → "code-action" **เสมอ** — "ตรวจสอบ/เช็ค" + อาการพัง = สั่งดูโค้ด ไม่ใช่ถามข้อมูล
2. ขอให้แอป/ระบบเปลี่ยนจากเดิม — แก้/เพิ่ม/ลบ/เปลี่ยน/ปรับ/ย้าย รวมถึงรูปประโยคอ้อมๆ "อยากให้...", "ช่วยเพิ่ม...หน่อย", "ขอ...ได้ไหม" ที่ผลลัพธ์คือโค้ดต้องแก้ → "code-action"
3. ถามข้อมูล/ยอดขาย/ขอสรุปตัวเลข/ขอความเห็น/ขอคำอธิบาย/ทักทาย/ขอบคุณ (ระบบปกติดี ไม่ได้ขอให้อะไรเปลี่ยน) → "chat"
...
- "Project Tree โหลดไม่ได้ ตรวจสอบหน่อย" → code-action(ข้อ1 — "โหลดไม่ได้"=ระบบพัง "ตรวจสอบ"≠ถามข้อมูล)
- "เช็คหน่อยว่าทำไมอัพโหลดรูปไม่ได้" → code-action(ข้อ1 — "อัพโหลดไม่ได้"=ระบบพัง) | "เช็คยอดขายวันนี้หน่อย" → chat(ข้อ3)
- "ระบบแจ้งเตือนพังอีกแล้ว ตรวจสอบที" → code-action(ข้อ1 — "พัง"=ระบบพัง ชัดเจน)
ไม่เข้าข้อไหนเลย → "chat"

กรณี code-action (แก้/เพิ่ม/ดูโค้ดจริง — รวมถึงรายงานบั๊ก/ขอตรวจสอบตามข้อ 1):
{
  "intent": "code-action",
...
กรณี chat (เข้าข้อ 3 ของขั้นตอนตัดสิน):
{"intent":"chat"}`;
  ...
  try {
    const res = await fetch(`${OPENROUTER_BASE}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0, // classifier ต้อง deterministic — 0.1 เดิมทำผล eval แกว่งระหว่างรอบ
        max_tokens: 900,
      }),
    });
