...
function buildFileSelectionPrompt(scopeInfo, message, agentDocs) {
  return `คุณคือจีจี้ — Senior Full-stack Developer ของ CHINCHA FLOW monorepo
...
function buildFixPlanPrompt(scopeInfo, message, fileContents, agentDocs) {
  ...
  return `คุณคือจีจี้ — Senior Full-stack Developer ของ CHINCHA FLOW monorepo
คุณกำลังแก้โค้ดตามคำสั่งของพี่พีช (เจ้าของร้าน) — Scope: ${scopeInfo.label}
...
async function openPR(pat, branchName, prTitle, prBody) {
  const res = await fetch(`${GH_API}/repos/${GH_REPO}/pulls`, {
    ...
    body: JSON.stringify({
      ...
      head: branchName,
      base: 'main',
      body: (prBody || `Auto-generated by จีจี้ (AI Workflow Agent)\n\nBranch: ${branchName}`) +
...
async function executeCodeAction(openRouterKey, ghPat, { message, history, scope }) {
  ...
  const codeModel = isComplex
    ? (process.env.CODE_MODEL || PRO_MODEL)
