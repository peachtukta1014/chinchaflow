...
const FLASH_MODEL = 'deepseek/deepseek-v4-flash';   // classifier เท่านั้น (ตัวจัดเส้นทาง ยิงทุกข้อความ) — ถูก เร็ว
const PRO_MODEL   = 'deepseek/deepseek-v4-pro';     // แชทที่ตอบพีช + เขียนโค้ด — โมเดลหลักตัวเดียว
...
// แชทที่ตอบพีชจริงๆ ใช้ deepseek-v4-pro ตัวเดียว (เลิกแยก flash/pro ตามคีย์เวิร์ด
// เพื่อความสม่ำเสมอ) — มีรูปแนบใช้ vision model (gpt-4o-mini) เหมือนเดิม
// (flash ยังใช้อยู่ที่ classifyAndTranslate ซึ่งเป็นตัวจัดเส้นทางเบื้องหลัง)
function pickModel(text, { imageBase64, images } = {}) {
  ...
  return process.env.CODE_MODEL || PRO_MODEL;
}
