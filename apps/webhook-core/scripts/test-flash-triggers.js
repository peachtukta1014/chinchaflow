...
// ── detectScope — ตรวจ scope จากข้อความ ──
const { detectScope } = require('../src/flash/flashPrompts');
assert(detectScope('แก้แอปกุ้งหน่อย', 'root') === 'seafood', 'detectScope: กุ้ง → seafood');
assert(detectScope('ร้านชามีปัญหา', 'root') === 'tea', 'detectScope: ชา → tea');
assert(detectScope('webhook พัง', 'root') === 'webhook', 'detectScope: webhook → webhook');
assert(detectScope('ระบบ LINE พัง', 'root') === 'webhook', 'detectScope: LINE → webhook');
assert(detectScope('สั่งซื้อออนไลน์', 'root') === 'root', 'detectScope: ออนไลน์ → root (ไม่ใช่ webhook)');
assert(detectScope('สวัสดีครับ', 'root') === 'root', 'detectScope: ทั่วไป → root');
